...
app.get('/api/orders', async (req, res) => {
  try {
    const orders = await Order.find()
      .populate('customerId', 'name') // Populate customer name if needed
      .select('orderId address products totalWithTax created'); // Include address in the selection
    res.json(orders);
  } catch (error) {
            ...
            <div className="d-flex flex-column flex-lg-row mb-3">
              ...
              <div className="d-flex flex-column flex-xl-row justify-content-between flex-1 ml-lg-2 mr-xl-4 p-3">
                <div className="order-details">
                  ...
                  <div className="mb-1">
                    <span>Address</span>
                    <span className="order-label">{` ${order.address?.street}, ${order.address?.city}, ${order.address?.state}, ${order.address?.zip}, ${order.address?.country}`}</span>
                  </div>
                </div>
              </div>
            </div>
